...
class Characters extends Entity implements Moveable {
    ...
    public Characters(String name, int x, int y, char symbol) {
        super(name);
        this.x = x;
        this.y = y;
        this.symbol = symbol;
    ...
    public void setLocation(boolean tumb){// параметр показывает нужна ли проверка на горизонтальность плоскости
        x = this.getX();
        y = this.getY();
        if (tumb || this.checkLocation(x, y)){
            char[][] temp = Place.getLocationWithCharacters();
            temp[y][x+3] = this.getSymbol();
            Place.setLocationWithCharacters(temp);
        }
    }
    public void move(int newX, int newY, boolean tumb) { //третий параметр показывает нужна ли проверка на горизонтальность плоскости
        if ( tumb){
